...
| **11** | **`chmod: cannot access '.github/scripts/...': No such file or directory`** | `actions/checkout` checked out a tenant branch (or legacy ref) that does not contain `.github/scripts/` or `.github/cf-apps.json`. | Never check out tenant branches into the runner workspace without restoring scripts. Workflows must include an `Ensure Workflow Automation Scripts` step that fetches `.github/scripts` and `.github/cf-apps.json` from the default branch. |
| **12** | **`$'\r': command not found` or silent exit code 1 in bash scripts** | Shell scripts (`.sh`) saved on Windows machines with CRLF (`\r\n`) line endings cause Linux runners to fail parsing. | Keep `.gitattributes` configured with `*.sh text eol=lf`. Always ensure bash scripts use LF (`\n`) line endings, and ensure sourced scripts end with `true` so trailing conditional checks don't return code 1. |
